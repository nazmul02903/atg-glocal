...
import { S3_CONFIG_WORKSHOP } from "../../../../constants/variables";
...
const WorkshopEvent = (props) => {
  ...
  const {
    ...
    control,
    formState: { errors },
  } = useForm({ resolver: yupResolver(validationSchemaWorkshopEvent) });
  ...
  const handleInputChange = (e, index) => {
    ...
    const list = [...speakerDetails];
    list[index][name] = value;
    setspeakerDetails(list);
  };
  ...
  const uploadImage = (e) => {
    S3FileUpload.uploadFile(e.target.files[0], S3_CONFIG_WORKSHOP)
  ...
  const onSubmit = (values) => {
    ...
    UserService.createWorkshopEvent(data)
      .then((res) => {
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className="form-group">
        ...
        <Controller
          ...
          rules={{ required: true }}
          className={
            errors.topicsCovered ? "form-control is-invalid" : "form-control"
          }
          render={(field) => <MyEditor {...field} errors={errors} />}
        />
        <small className="text-danger">
          {errors.topicsCovered && errors.topicsCovered.message}
        </small>
      </div>
      ...
      <div className="form-group">
        <label htmlFor="totalSeats">
          Seat Limit
          <span className="text-danger">
            <em>( *Note - Enter 0 if no Seat Limit )</em>
          </span>
        </label>
        <input
          {...register("totalSeats")}
...
export default connect(mapStateToProps)(WorkshopEvent);
